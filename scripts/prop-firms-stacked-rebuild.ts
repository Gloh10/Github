...
// New baseline: flagship + VA filter (lookback=40) + daily-open magnet target (1-8R) --
// validated to stack: best cross-market OOS and lowest drawdown of anything tested this session.
function buildStackedTrades(bars: Bar[], deadlineHour: number, deadlineMinute: number): Trade[] {
...
const lucidTrades = buildStackedTrades(nq5m, 16, 45);
...
const mffTrades = buildStackedTrades(nq5m, 16, 10);
...
const apexTrades = buildStackedTrades(nq5m, 16, 59);
...
function main() {
  console.log("=".repeat(100));
  console.log("Prop-firm eval + payout rebuild with the STACKED baseline: flagship + VA filter + daily-open target");
  console.log("=".repeat(100));
  ...
  const lucidSummary = report(
    "LucidFlex 150K (STACKED)",
    lucidResults,
    "unfiltered: cleared 99.6%, avg payouts 0.83, expected $2,484   |   VA-only: cleared 100.0%, avg payouts 0.69, expected $2,063",
  );
  ...
  const mffSummary = report(
    "MyFundedFutures Pro 150K (STACKED)",
    mffResults,
    "unfiltered: cleared 97.8%, busted 3.9%, avg payouts 0.54, expected $3,617   |   VA-only: cleared 99.7%, busted 1.1%, avg payouts 0.22, expected $1,295",
  );
  ...
  const apexSummary = report(
    "Apex 150K (STACKED)",
    apexResults,
    "unfiltered: cleared 99.9%, busted 0.1%, avg payouts 1.11, expected $2,837   |   VA-only: cleared 100.0%, busted 0.1%, avg payouts 0.86, expected $2,158",
  );

  writeFileSync(
    "data/prop-firms-stacked-rebuild-results.json",
  ...
  console.log("Full results written to data/prop-firms-stacked-rebuild-results.json");
}
