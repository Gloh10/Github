...
// New baseline: flagship + "outside value area" filter (lookback=40), validated to hold up
// in-sample, split-period, cost-adjusted, and out-of-sample on two independent markets.
function buildVaFilteredTrades(bars: Bar[], deadlineHour: number, deadlineMinute: number): Trade[] {
...
const lucidTrades = buildVaFilteredTrades(nq5m, 16, 45);
...
const mffTrades = buildVaFilteredTrades(nq5m, 16, 10);
...
const apexTrades = buildVaFilteredTrades(nq5m, 16, 59);
...
const APEX_QUALIFYING_DAY_MIN_PROFIT = 350; // verified: 150K tier's per-day qualifying threshold
...
function main() {
  console.log("=".repeat(100));
  console.log("Prop-firm eval + payout rebuild with the NEW baseline: flagship + outside-value-area filter (lookback=40)");
  console.log("=".repeat(100));
  ...
  const lucidSummary = report("LucidFlex 150K (VA-filtered)", lucidResults, "old (realistic, no VA filter): cleared 99.6%, avg days 8.1, avg payouts 0.83, P(>=1)=82.9%, expected $2,484");
  ...
  const mffSummary = report("MyFundedFutures Pro 150K (VA-filtered)", mffResults, "old (realistic, no VA filter): cleared 97.8%, busted 3.9%, avg payouts 0.54, expected $3,617");
  ...
  const apexSummary = report("Apex 150K (VA-filtered)", apexResults, "old (realistic, no VA filter): cleared 99.9%, busted 0.1%, avg payouts 1.11, expected $2,837");

  writeFileSync(
    "data/prop-firms-va-filtered-rebuild-results.json",
  ...
  console.log("Full results written to data/prop-firms-va-filtered-rebuild-results.json");
}
