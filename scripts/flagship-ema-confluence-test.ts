...
import { ema, sessionVwap } from "../src/backtest/indicators.js";
...
type FilterName = "none" | "ema21" | "ema50" | "ema200" | "cross21_50";
...
function buildSignals(bars: Bar[], filter: FilterName): Signal[] {
  ...
  return filtered.filter((s) => {
    const bar = bars[s.barIndex]!;
    if (filter === "ema21") {
      if (isNaN(ema21[s.barIndex]!)) return false;
      return s.direction === "long" ? bar.c > ema21[s.barIndex]! : bar.c < ema21[s.barIndex]!;
    }
    if (filter === "ema50") {
      if (isNaN(ema50[s.barIndex]!)) return false;
      return s.direction === "long" ? bar.c > ema50[s.barIndex]! : bar.c < ema50[s.barIndex]!;
    }
    if (filter === "ema200") {
      if (isNaN(ema200[s.barIndex]!)) return false;
      return s.direction === "long" ? bar.c > ema200[s.barIndex]! : bar.c < ema200[s.barIndex]!;
    }
    if (filter === "cross21_50") {
      if (isNaN(ema21[s.barIndex]!) || isNaN(ema50[s.barIndex]!)) return false;
      return s.direction === "long" ? ema21[s.barIndex]! > ema50[s.barIndex]! : ema21[s.barIndex]! < ema50[s.barIndex]!;
    }
    return true;
  });
}
...
function main() {
  ...
  const filters: FilterName[] = ["none", "ema21", "ema50", "ema200", "cross21_50"];
  const labels: Record<FilterName, string> = {
    none: "No EMA filter (baseline)",
    ema21: "Price vs EMA(21) trend alignment required",
    ema50: "Price vs EMA(50) trend alignment required",
    ema200: "Price vs EMA(200) trend alignment required",
    cross21_50: "EMA(21) vs EMA(50) crossover alignment required",
  };

  console.log("=".repeat(110));
  console.log("EMA trend-alignment filters on the flagship strategy");
  console.log("=".repeat(110));
  ...
  for (const f of filters) {
    ...
    fullResults[f] = r;
    console.log(`${labels[f].padEnd(46)} ${summarize(r)}`);
  }
  ...
  for (const f of filters) {
    ...
    console.log(`${labels[f].padEnd(46)} H1 totalR=${h1.stats.totalR.toFixed(1).padStart(6)}  H2 totalR=${h2.stats.totalR.toFixed(1).padStart(6)}  ${bothPositive ? "POSITIVE both" : "NOT both positive"}`);
  ...
  for (const f of filters) {
    ...
    console.log(`${labels[f].padEnd(46)} totalR(noCost)=${full.stats.totalR.toFixed(1).padStart(6)}  totalR(cost)=${stats.totalR.toFixed(1).padStart(6)}`);
  ...
  for (const f of filters) {
    ...
    console.log(`${labels[f].padEnd(46)} ${summarize(oos)}`);
    oosResults.push({ filter: f, ...oos.stats });
  }
  output.outOfSample = oosResults;

  writeFileSync("data/flagship-ema-confluence-test-results.json", JSON.stringify(output, null, 2));
  console.log("\nFull results written to data/flagship-ema-confluence-test-results.json");
}
