...
const VA_LOOKBACK = 40;
const DAILY_OPEN_MIN_R = 1;
const DAILY_OPEN_MAX_R = 8;
...
function applyDailyOpenTarget(bars: Bar[], signals: Signal[]): Signal[] {
  ...
  return signals.map((s) => {
    ...
    if (levelAhead && levelR >= DAILY_OPEN_MIN_R && levelR <= DAILY_OPEN_MAX_R) {
...
function applyVaFilter(bars: Bar[], signals: Signal[]): Signal[] {
  const va = rollingValueArea(bars, VA_LOOKBACK);
  return signals.filter((s) => {
...
type ConfigName = "baseline" | "vaOnly" | "dailyOpenOnly" | "both";
...
function build(bars: Bar[], config: ConfigName): StrategyResult {
  ...
  if (config === "vaOnly") {
    signals = overrideTargetR(applyVaFilter(bars, signals), 5);
  } else if (config === "dailyOpenOnly") {
    signals = applyDailyOpenTarget(bars, signals);
  } else if (config === "both") {
    signals = applyDailyOpenTarget(bars, applyVaFilter(bars, signals));
  } else {
...
function main() {
  ...
  const configs: ConfigName[] = ["baseline", "vaOnly", "dailyOpenOnly", "both"];
  const labels: Record<ConfigName, string> = {
    baseline: "Baseline (fixed R=5, no filters)",
    vaOnly: "VA filter only (lookback=40)",
    dailyOpenOnly: "Daily-open target only (1-8R)",
    both: "STACKED: VA filter + daily-open target",
  };

  console.log("=".repeat(115));
  console.log("Do the VA filter and daily-open magnet target stack?");
  console.log("=".repeat(115));
  ...
  for (const c of configs) {
    ...
    fullResults[c] = r;
    console.log(`${labels[c].padEnd(42)} ${summarize(r)}`);
  }
  ...
  for (const c of configs) {
    ...
    console.log(`${labels[c].padEnd(42)} H1 totalR=${h1.stats.totalR.toFixed(1).padStart(6)}  H2 totalR=${h2.stats.totalR.toFixed(1).padStart(6)}  ${bothPositive ? "POSITIVE both" : "NOT both positive"}`);
  ...
  for (const c of configs) {
    ...
    console.log(`${labels[c].padEnd(42)} totalR(noCost)=${full.stats.totalR.toFixed(1).padStart(6)}  totalR(cost)=${stats.totalR.toFixed(1).padStart(6)}`);
  ...
  writeFileSync("data/flagship-va-plus-dailyopen-stack-test-results.json", JSON.stringify(output, null, 2));
  console.log("\nFull results written to data/flagship-va-plus-dailyopen-stack-test-results.json");
}
