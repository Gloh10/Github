...
type ConfigName = "baseline" | "meanRevOnly";
...
function build(bars: Bar[], config: ConfigName): StrategyResult {
  ...
  const rawSignals = config === "baseline" ? merge(meanRev, trend) : meanRev;
  const signals = overrideTargetR(dayHourFilter(bars, rawSignals), 5);
...
function main() {
  ...
  const configs: ConfigName[] = ["baseline", "meanRevOnly"];
  const labels: Record<ConfigName, string> = {
    baseline: "Baseline (mean-rev + trend, current flagship)",
    meanRevOnly: "MEAN-REV ONLY (drop the trend-continuation leg entirely)",
  };

  console.log("=".repeat(120));
  console.log("Diagnostic: does dropping the trend-continuation leg improve the flagship?");
  console.log("=".repeat(120));
  ...
  for (const c of configs) {
    ...
    console.log(`${labels[c].padEnd(58)} H1 totalR=${h1.stats.totalR.toFixed(1).padStart(6)} (n=${h1.stats.totalTrades})  H2 totalR=${h2.stats.totalR.toFixed(1).padStart(6)} (n=${h2.stats.totalTrades})  ${bothPositive ? "POSITIVE both" : "NOT both positive"}`);
  ...
  writeFileSync("data/flagship-drop-trend-leg-test-results.json", JSON.stringify(output, null, 2));
  console.log("\nFull results written to data/flagship-drop-trend-leg-test-results.json");
}
